...
namespace PuppetFace
{
	public class PuppetFaceEditor : EditorWindow
	{
        ...
        private void ConvertAudioToLipSyncFolder()
        {
			...
			List<string> wavFiles = new List<string>(files);
		...
		private void ConvertAudioToLipSyncAll(string[] files)
		{
            ...
            */
			...
			if (files.Length != 0)
			{
				...
				{
					// Only handle files inside the Unity Assets folder to ensure AssetDatabase access
					if (!path.StartsWith(Application.dataPath))
					{
						Debug.LogWarning("Skipping file outside Assets: " + path);
						continue;
					}
                    ...
                    if (LipSyncLoaded != null)
					{
						...
						if (textAsset == null || new FileInfo(relativepath.Replace("Assets", Application.dataPath)).Length == 0)
						{
							Debug.LogWarning("LipSync XML missing or empty for: " + relativepath + ". Skipping association.");
							continue;
						}
						...
						else
						{
							...
							addedIndex = lipSyncFiles.Count - 1;
						}
						...
						LipSyncLoaded.Initialized = false;
						// If XML exists, reload from file; otherwise keep current data
						if (LipSyncLoaded.LipSyncFiles != null && LipSyncLoaded.LipSyncFiles.Length > addedIndex && LipSyncLoaded.LipSyncFiles[addedIndex] != null)
						{
		...
		private void ReloadNodes(Vector2 mousePosition)
		{
			...
			if (AudioSourceLoaded && LipSyncLoaded && AudioClipLoaded && LipSyncLoaded.LipSyncFiles.Length > 0)
			{
				...
				_phonemeMarkers.Clear();
					if (LipSyncLoaded.LipSyncFiles[LipSyncLoaded.LipSyncIndex] == null)
						return;

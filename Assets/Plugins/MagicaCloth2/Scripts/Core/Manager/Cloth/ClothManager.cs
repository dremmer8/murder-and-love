...
namespace MagicaCloth2
{
    ...
    public class ClothManager : IManager, IValid
    {
        ...
        static bool _webglLoggedNoTeams = false;
        static bool _webglLoggedSimulating = false;
#endif
        ...
        void ClothUpdate()
        {
            ...
            {
#if UNITY_WEBGL && !UNITY_EDITOR
                if (!_webglLoggedNoTeams)
                {
                    _webglLoggedNoTeams = true;
                    Debug.Log("[MagicaCloth2 WebGL] No active cloth teams - simulation skipped. Ensure: 1) Use Pre-Build data for WebGL (bake in Editor), 2) MagicaCloth components are enabled, 3) Build completed without errors.");
                }
#endif
...
#if UNITY_WEBGL && !UNITY_EDITOR
            if (!_webglLoggedSimulating)
            {
                _webglLoggedSimulating = true;
                Debug.Log($"[MagicaCloth2 WebGL] Simulating with {tm.ActiveTeamCount} active team(s).");
            }
#endif
            ...
            {
#if UNITY_WEBGL && !UNITY_EDITOR
                Debug.LogError($"[MagicaCloth2 WebGL] Simulation error: {e.Message}\n{e.StackTrace}");
#endif
                throw;
            }
